...
import {
  ...
  deployWorkflow,
  getLatestRunIdFor,
} from "../githubaction/workflow";
import { workflowStatusAttachment } from "./message";
import { checkPermission } from "./permission";
import { deleteMessage, postApproveOrDenyMessage } from "./web-client";
import {
  decodeDeployActionValue,
  decodeRequestActionValue,
} from "../githubaction/actionvalue";
...
boltApp.action("execute", async ({ body, ack }) => {
  ...
  if (body.type === "block_actions") {
    ...
    if (action.type === "button" && body.message && body.channel) {
      ...
      await postApproveOrDenyMessage(
        workflowId,
        ref,
...
boltApp.action("approve", async ({ body, ack, say }) => {
  ...
  try {
    ...
    const { workflowId, ref, requester } = decodeRequestActionValue(
      action.value
    );
    await postApproveOrDenyMessage(
      workflowId,
    ...
    while (status != WorkflowStatus.COMPLETE) {
      [status, conclusion, create_at, update_at] = await checkWorkflowStatus(
        runId
      );
      const duration =
        (new Date(update_at).getTime() - new Date(create_at).getTime()) / 1000;

      if (prevStatus != status) {
        if (message != "") {
          await deleteMessage(message.ts, message.channel);
        }
        const [title, attachments] = await workflowStatusAttachment(
          workflowId,
          runId,
          ref,
          create_at,
